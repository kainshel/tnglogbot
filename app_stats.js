...
function render(){
  ...
  DB.workouts.forEach(w=>{
    ...
    if(!byMonth.has(key)) byMonth.set(key,{count:0,vol:0,ex:new Set()});
    const m=byMonth.get(key); m.count++; m.vol+=vol; exSet.forEach(e=>m.ex.add(e));
  });
  statsMonthly.innerHTML='';
  Array.from(byMonth.entries()).sort((a,b)=>a[0]<b[0]?-1:1).forEach(([k,v])=>{
    const tr=document.createElement('tr'); tr.innerHTML=`<td>${k}</td><td>${v.count}</td><td>${v.vol}</td><td>${v.ex.size}</td>`;
