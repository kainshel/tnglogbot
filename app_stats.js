...
function render(){ const byMonth=new Map(); DB.workouts.forEach(w=>{ const key=w.date.slice(0,7);
  ...
  statsMonthly.innerHTML=''; Array.from(byMonth.entries()).sort((a,b)=>a[0]<b[0]?-1:1).forEach(([k,v])=>{ const tr=document.createElement('tr'); tr.innerHTML=`<td data-label="Месяц">${k}</td><td data-label="Тренировок">${v.count}</td><td data-label="Общий объём">${v.vol}</td><td data-label="Уникальных упражнений">${v.ex.size}</td>`; statsMonthly.appendChild(tr); }); }
render();
