...
const dayCard = document.getElementById('dayWorkoutsCard');
const dayTitle = document.getElementById('dayTitle');
const dayTbody = document.getElementById('dayWorkouts');
...
function renderCalendar(){
  ...
  for(let i=0;i<startDay;i++){ calGrid.appendChild(empty()); }
...
function showDay(dateStr, list){
  dayCard.hidden=false; dayTitle.textContent=`Тренировки за ${dateStr}`; dayTbody.innerHTML='';
  list.forEach(w=>{
    const tr=document.createElement('tr');
    tr.innerHTML=`<td>${w.date}</td><td>${totalVolume(w)}</td>`;
    const td=document.createElement('td');
    const open=document.createElement('a'); open.className='icon-btn'; open.textContent='Открыть'; open.href=`view.html?id=${w.id}`;
    const del=document.createElement('button'); del.className='icon-btn'; del.textContent='Удалить';
    del.onclick=()=>{ if(confirm('Удалить?')){ DB.workouts=DB.workouts.filter(x=>x.id!==w.id); renderCalendar(); dayCard.hidden=true; } };
